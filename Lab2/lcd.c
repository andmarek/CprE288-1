 ...
 * @date
 */
void  d_rotatingBanner(char dispText[]){
    ...
    while(1){
                 ...
                 // timer_waitMillis(300);
                  ...
                  char msg[100];
                  int k;
                  for( k =0; k< strlen(dispText)+1; k++){ // Copys over Display Text and puts it into msg[]
                      msg[k] = dispText[k];
                  }
                  char space[] = "                    ";// Use These spaces to start off

                  char temp[100] = "";

                  strcat(temp, space);
                                      //These are used to move the words in display text into the temp array
                  strcat(temp, msg);

                  int i =0;

                  for(i =0; i <strlen(temp)+1; i++){//Adds The new value moving into the temp array and prints it

                               lcd_printf("%.20s", temp+i);
...
/**
    lcd_init();
            ...
            char Space[21] = "                    "; //Space used to have a blank screen at the start
            int i;
            int j;

            for(i = 0; i < size; i++){ //Move the space to the left and the words are replacing it
               for(j = 0; j < 20; j++){
                   Space[j] = c[j+i];
               }

               lcd_printf(Space);// It Prints the space but the words are going inside the space
               timer_waitMillis(300);

               if(i == size-1){ // This will resart when I has reached the size of the word
                   i = 0; // reset
