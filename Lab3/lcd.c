...
void lcd_rotatingBanner(char dispText[]){
    ...
    for( i = 0; i < len; i++){
        myString[i] = dispText[i];
    }

    //add a null character at the last space
    myString[len] = '\0';

    // String currently being displayed.
    char displayString[21] = "                    ";

    // keeps track of current index in array
    int stringCounter = 0;
    int stringLength = strlen(myString);

    //Count spaces printed after the message is printed
    int spaceCounter = 0;
    ...
    while(1){
        ...
        //shift all characters one to the left
        for (i = 0; i < 19; i++){
            displayString[i] = displayString[i + 1];
        }

        // Adds a character from myString, or a space if at end
        if (stringCounter < stringLength){
            displayString[19] = myString[stringCounter];
            stringCounter++;
        }
